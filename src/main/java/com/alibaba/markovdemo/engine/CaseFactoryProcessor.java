...
public class CaseFactoryProcessor {
    ...
    @Value("${markov.ai.engine}")
    public void setAiEngine(String engineName) {
        this.aiEngine = context.getBean(engineName, AIEngine.class);
    }
    ...
    public List<String> extractFeatures(GotTestCase gotTestCase) {
        ...
        List<String> featureList = new ArrayList<>();
        //用例名进行FMM分词
        List<String> descriptionFeatures = aiEngine.getFeatures(gotTestCase.getDescription());
        //用例描述FMM分词
        List<String> longDescriptionFeatures = aiEngine.getFeatures(gotTestCase.getLongDescription());
        //分组特征
     ...
     * @param scenarioId
     */
    public void callFeaturesExtract(Long scenarioId) throws Exception {
        ...
        for (GotTestCase gotTestCase : gotTestCaseList) {
            try {
                ...
                logger.debug("用例id: {} 特征抽取完成!", gotTestCase.getId());
            } catch (Exception e) {
                logger.error("用例id: {} 特征抽取过程出错,请注意!", gotTestCase.getId(), e);
            }
        }
        //特征池入库
        ...
        logger.info("特征池入库完成: {} 个特征", featurePool.size());
    }
    ...
    // 升序比较器
    private Comparator<Map.Entry<String, Double>> valueComparator = new Comparator<Map.Entry<String, Double>>() {
        @Override
    ...
    // 升序比较器
    private Comparator<Map.Entry<GotTestCase, Float>> valueCaseComparator = new Comparator<Map.Entry<GotTestCase, Float>>() {
        @Override
        public int compare(Map.Entry<GotTestCase, Float> o1,
                           Map.Entry<GotTestCase, Float> o2) {
    ...
    public JSONObject featuresMatch(FeatureInfo featureInfo) throws IOException {
        ...
        Map<GotTestCase, Float> map = new HashMap<>();
        List<TestCaseInput> sortList = new ArrayList<>();

        //对业务描述进行分词,并合并入匹配特征列表
        if (featureInfo.getCaseDesc() != null) {
            List<String> descriptionFeatures = aiEngine.getFeatures(featureInfo.getCaseDesc());
        ...
        for (GotTestCase gotTestCase : gotTestCaseList) {
            try {
                //命中特征的数目
                float num = aiEngine.sim(gotTestCase, featureInfo);
                map.put(gotTestCase, num);
                logger.debug("用例: {} 特征匹配完成!", gotTestCase);
            } catch (Exception e) {
                // ignore 匹配失败了就忽略这一个测试用例
            }
        }

        List<TestCaseInput> list = Lists.newArrayList(map.entrySet()).stream().sorted(valueCaseComparator).limit(10).
                map(entry -> TestcaseService.formatTestCaseInput(entry.getKey())).collect(Collectors.toList());

        Date end = new Date();
        res.put("caseList", list);
        res.put("caseCount", list.size());
        res.put("interval", end.getTime() - start.getTime());
        res.put("recommenderName", aiEngine.getEngineName());
        return res;
    }
